...
public class Agency {
	...
	public void definePoints(double... points) throws AgencyException {
		...
		for(int i=0;i<points.length-1;i++) {
			if(points[i]>points[i+1])
					throw new AgencyException("not ascending");
		}
	...
	public double getPointsOfCategory(char category) throws AgencyException {
		...
		int index=category-conv;
		if(index<0 || index>points.length) {
			throw new AgencyException("not define category");
		}
	...
	public int addCar(String manufacturer, String model, int year, String gear, String color, char category, int seats) throws AgencyException {
		...
		vehicleCounter++;
		Car c=new Car(vehicleCounter, manufacturer, model, year, gear, color, category, seats);
		vehicleList.add(c);
		this.getPointsOfCategory(category);
	...
	public int addVan(String manufacturer, String model, int year, String gear, String color, char category, int seats,
		...
		vehicleCounter++;
		Van v=new Van(vehicleCounter, manufacturer, model, year, gear, color, category, seats, limit);
		vehicleList.add(v);
		this.getPointsOfCategory(category);
		return vehicleCounter;
	}
	...
	public List<String> getVehiclesOfAManufacturer(String manufacturer) {
		...
		return vehicleList.stream().filter(v->v.manufacturer.equals(manufacturer))
		.sorted(Comparator.comparingInt(v->v.id)) //non serve perchè gia ordianto
		.map(v->v.getInfo())
	...
	public Map<String, List<String>> getUserInfo() {
		...
		List<User> s=new ArrayList<>(userList);
		...
		for(User u: s) {
			if(m.containsKey(u.getCity())) {
				List<String> h=m.get(u.getCity());
				h.add(u.getName());
		...
		/*return userList.stream().sorted(Comparator.comparing(User::getName).reversed())
		.collect(Collectors.groupingBy(User::getCity,TreeMap::new,
