...
//R1
pm.addBuilding("b1",20); pm.addBuilding("b3",100); pm.addBuilding("b2",20);
try {pm.addBuilding("b1",50);
	} catch(Exception e) {print("building duplicated");}

pm.addOwner("ow1","b1:1","b2:5","b3:5"); pm.addOwner("ow2","b3:1","b2:10");
pm.addOwner("ow3","b1:2"); pm.addOwner("ow8","b1:18"); 
try {pm.addOwner("ow3","b1:2");
} catch(Exception e) {print("the apartment already has the owner");}
...
try {pm.addProfessionals("decorator", "mas1");
} catch(Exception e) {print("professional duplicated");}
...
pm.assign(r3,"pl1");
pm.assign(r5,"pl1");
try {pm.assign(r2,"mas1");
} catch(Exception e) {print("wrong professional");}
...
pm.charge(r2, 200); 
try {pm.charge(6,900);
} catch(Exception e) {print("undefined request");}
